...
(async () => {
   ...
   *
   */
  const shadeGeometry = new THREE.BoxGeometry(200, 200, 10);
  const shadeMaterial = new THREE.MeshBasicMaterial({ color: 0x0000ff });
  const shadeMesh = new THREE.Mesh(shadeGeometry, shadeMaterial);
  // currently magic numbers
  shadeMesh.position.set(130, 0, 50);
  shadeMesh.rotateY(Math.PI / 8);
  const node5 = new SDV.TreeNode();
  node4.addChild(node5);
  node5.addData(new SDV.ThreejsData(shadeMesh));
