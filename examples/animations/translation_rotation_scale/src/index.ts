...
(async () => {
  ...
  const horizontalTopOutput = session
    .getOutputByName("HorizontalTop")
    .find((o) => !!o.material);
  const horizontalTopNode = session.node.children.find(
    (c) => c.name === horizontalTopOutput!.id
  )!;

  const horizontalMidOutput = session
    .getOutputByName("HorizontalMid")
    .find((o) => !!o.material);
  const horizontalMidNode = session.node.children.find(
    (c) => c.name === horizontalMidOutput!.id
  )!;

  const horizontalBottomOutput = session
    .getOutputByName("HorizontalBottom")
    .find((o) => !!o.material);
  const horizontalBottomNode = session.node.children.find(
    (c) => c.name === horizontalBottomOutput!.id
  )!;
  ...
  const tracks2: IAnimationTrack[] = [
    {
      times: [0, 5, 10],
      node: horizontalTopNode,
    ...
    {
      times: [0, 5, 10],
      node: horizontalMidNode,
      values: [1, 1, 1, 0.5, 0.5, 0.5, 1, 1, 1],
      path: "scale",
      interpolation: "linear"
    },
    {
      times: [0, 5, 10],
      node: horizontalBottomNode,
      values: [1, 1, 1, 2, 2, 2, 1, 1, 1],
      path: "scale",
      interpolation: "linear"
    }
  ];
  const data2 = new AnimationData("myAnimation", tracks2, 4, 14);

  session.node.data.push(data1);
  data1.repeat = true;
  data1.startAnimation();

  session.node.data.push(data2);
  data2.repeat = true;
