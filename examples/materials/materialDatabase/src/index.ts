...
*/
const cachedMaterials: { [key: string]: { name: string, materialData: IMaterialStandardData | IMaterialGemData } } = {};
    ...
    const cb = (newNode?: ITreeNode) => {
        ...
        for (const key in materialDatabaseDefinition) {
            ...
            if (cachedMaterials[cacheKey]) {
                promises.push(Promise.resolve({ name: key, materialData: cachedMaterials[cacheKey].materialData }));
            } else {
                promises.push(
                    ...
                    MaterialEngine.instance.createMaterialDataFromDefinition(materialDatabaseDefinition[key])
                        .then((materialData) => {
                            cachedMaterials[cacheKey] = { name: key, materialData };
        ...
        // Clear the cached materials that are not used anymore.
        for (const cacheKey in cachedMaterials) {
            if (!cacheKeys.includes(cacheKey)) {
                delete cachedMaterials[cacheKey];
            }
        }
