...
(async () => {
  ...
  // create a callback that is executed whenever the output changes
  const cb = (newNode?: ITreeNode, oldNode?: ITreeNode) => {
    if (newNode) {
  ...
  // call it once to apply the changes immediately
  cb(doorOutput.node);

  // by setting this callback the door output will be adjusted whenever the node is replaced
  doorOutput.updateCallback = cb;
})();
